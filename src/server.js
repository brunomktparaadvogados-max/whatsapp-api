...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (existingSession) {
      ...
      if (dbSession && hasRemoteAuth && !forceQrRequired && needsLiveSessionReconnect(dbSession.status)) {
    ...
    if (session) {
      ...
      if (dbSession) {
        ...
        sessionInfo = {
          ...
          status: (!recoverable && isQrRequiredDbStatus(dbSession.status)) ? 'initializing' : status,
          qrCode: null,
          info: null,
          hasRemoteAuth,
          recoverable,
          reactivationStarted: reactivationStarted || (!recoverable && isQrRequiredDbStatus(dbSession.status))
        };
      }
    }
...
function kickSessionActivationWithQrFallback(sessionId, userId, dbSession = null, reason = 'saved_auth') {
  ...
  setImmediate(async () => {
    try {
      ...
      const canTrustSavedAuth = isSavedAuthTrusted(hasRemoteAuth, sessionRow.status, forceQr, sessionId);
      ...
      try {
        for (const session of toRecover) {
          ...
          try {
            ...
            const canTrustSavedAuth = isSavedAuthTrusted(hasRemoteAuth, session.status, forceQr, session.id);
  ...
  try {
    ...
    if (!session) {
      ...
      const reactivationStarted = recoverable
        ...
        : false;
      if (!recoverable && dbSession && isQrRequiredDbStatus(dbSession.status)) {
        setImmediate(() => {
      ...
      return res.json({
        success: true,
        sessionId,
        status: (!recoverable && dbSession && isQrRequiredDbStatus(dbSession.status)) ? 'initializing' : status,
        canSend: false,
        hasRemoteAuth,
        recoverable: !!recoverable,
        reactivationStarted: reactivationStarted || (!recoverable && dbSession && isQrRequiredDbStatus(dbSession.status)),
...
app.post('/api/sessions/:sessionId/reactivate', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        await db.updateSessionStatus(sessionId, canTrustSavedAuth ? 'auth_failure' : 'failed');
      } catch (_) { /* ignore */ }
      io.to(`user_${targetUserId}`).emit('session_error', {
        sessionId,
        error: error.message
          ...
          if (session.client) {
            ...
            try {
              ...
              if (!alive) {
                ...
                await db.updateSessionStatus(sid, 'auth_failure');
