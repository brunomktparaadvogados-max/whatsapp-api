...
app.get('/api/leads', authMiddleware, async (req, res) => {
  try {
    const leads = await db.getLeadsByUser(req.userId);
    res.json({
      success: true,
...
app.get('/api/leads/:contactPhone', authMiddleware, async (req, res) => {
  try {
    const { contactPhone } = req.params;
    const lead = await db.getLeadByContact(req.userId, contactPhone);
...
app.put('/api/leads/:contactPhone', authMiddleware, async (req, res) => {
  try {
    const { contactPhone } = req.params;
    const { stage, keyword } = req.body;

    if (!stage) {
      return res.status(400).json({ error: 'Campo "stage" é obrigatório' });
    }

    const validStages = ['new', 'qualified', 'contacted', 'won', 'lost'];
    ...
    await db.updateLeadStage(req.userId, contactPhone, stage, keyword || null);
